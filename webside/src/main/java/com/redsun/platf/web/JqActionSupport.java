...
// @Results( { @Result(name = CrudActionSupport.RELOAD, location =
// "jqfull-system-value" ) })

public abstract class JqActionSupport<T> extends CrudActionSupport<T> {
	...
	protected Page<T> gridModel = new Page<T>();// 每页5条记录
	 ...
	 * 显示
	 */
	// @Actions( { @Action(value = "/jqfull-system-value-json", results = {
	...
	public String list() throws Exception {
		...
		System.out.println("super:" + mainManager);
	...
	/**
	 * 实现prepare()函数,屏蔽了所有Action函数都会执行的公共的二次绑定.
	 */
	...
	public Page<T> getPageModel() {
		return gridModel;
	}

	public void setPageModel(Page<T> pageModel) {
